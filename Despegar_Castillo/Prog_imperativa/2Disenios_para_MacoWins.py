...
# El stock del producto agregado debe estar inicialmente en cero.
...
#2.
# recargar_stock: toma un código de producto y una cantidad de unidad de stock a agregar, e incrementa el stock correspondiente a ese producto.
# Si el código de producto indicado no existe, debe lanzar una excepción."""
...
#3.hay_stock: recibe un código de producto y dice si hay stock (es decir, si el stock correspondiente es mayor a cero).
# Si el código indicado no existe en la lista de productos, debe devolver False.
...
#6.realizar_compra: recibe un código de producto y una cantidad de items a comprar. En base a ello, decrementa el stock del producto correspondiente
# y crea una nueva venta con la información correspondiente. Si no hay suficiente stock, lanzar una excepción.
...
def discontinuar_productos(productos):
    x = 0
    while x < len(productos):
        if productos[x]["stock"] == 0:
            productos.remove(productos[x])
        else:
            x += 1
...
def valor_ventas_del_día(ventas):
    import datetime
    total_recaudado = 0
    for x in range(len(ventas)):
        fecha = datetime.strftime(ventas[x]["fecha"], '%d-%m-%Y')
...
def valor_ventas_del_año(ventas):
  import datetime
  total_recaudado = 0
  for x in range(len(ventas)):
        fecha = datetime.strptime(ventas[x]["fecha"], '%d-%m-%Y')
        if fecha.date().strftime("%Y") == datetime.today().date().strftime("%Y"):
            total_recaudado += ventas[x]["precio"]
  return total_recaudado
...
#11.actualizar_precios_por_categoria: toma una categoría y un porcentaje, y actualiza según ese porcentaje el precio de todos los productos 
#que tengan esa categoría. La búsqueda de categoría en este procedimiento no debe ser exacta: por ejemplo tanto si se pasa como 
